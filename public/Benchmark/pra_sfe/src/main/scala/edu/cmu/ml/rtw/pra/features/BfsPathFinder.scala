...
abstract class BfsPathFinder[T <: Instance](
  ...
  def shouldSkip(source: Int, target: Int, node1: Int, node2: Int, relation: Int, exclude: Set[Int]) = {
    //if (!(source == node1 && target == node2) && !(source == node2 && target == node1) && !(exclude.contains(relation)))
    // val tmp_1 = unallowedPairs.map(c => c._3)  
    // val tmp_2 = unallowedPairs.map(c => (c._1, c._2))  
    // // tmp.add(2)
    // // if  (tmp.contains(relation))     
    // if ((unallowedPairs.contains((node1, node2, relation))) || (unallowedPairs.contains((node2, node1, relation))) || ((tmp_1.contains(relation))  && ((tmp_2.contains((node1, node2)))  || (tmp_2.contains((node2, node1))) )))

    // val tmp = unallowedPairs.map(c => c._3)  
    // tmp.add(2)
